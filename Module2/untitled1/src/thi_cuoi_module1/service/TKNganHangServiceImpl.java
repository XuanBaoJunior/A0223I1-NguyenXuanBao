package thi_cuoi_module1.service;

import thi_cuoi_module1.controller.TKNganHangController;
import thi_cuoi_module1.model.TKThanhToan;
import thi_cuoi_module1.model.TKTietKiem;
import thi_cuoi_module1.repository.TKThanhToanRepository;
import thi_cuoi_module1.repository.TKTietKiemRepository;
...
public class TKNganHangServiceImpl implements Service{
    ...
    public void display() {
        ...
        for (TKTietKiem tkttList : tkTietKiemList){
            System.out.println(tkttList.toString());
        }
    }
    ...
    @Override
    public void remove() {
        System.out.println("Loai tai khoan ngan hang muon xoa \n" +
                "1.TK Thanh toan \n" +
                "2.TK Tiet kiem ");
        int choose = Integer.parseInt(scanner.nextLine());
        switch (choose) {
            case 1:
                System.out.println("Nhap ma tai khoan muon xoa ");
                int ten = scanner.nextInt();
                TKThanhToan tkThanhToan = null;
                for (int i = 0; i < tkThanhToanList.size(); i++) {
                    TKThanhToan tktt = tkThanhToanList.get(i);
                ...
                break;
            case 2:
                System.out.println("Nhập mã id tai khoan muon xoá ");
                int tenXT = scanner.nextInt();
